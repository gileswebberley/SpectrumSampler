...
void ofApp::seedCritters(){
    ...
    for(int i =0;i<clouds; i++){
        ...
        tx[i] = ofRandom(minBassOut,maxBassOut);
        ty[i] = ofRandom(minBassOut,maxBassOut);
    }
...
ofColor ofApp::updateColour(){
    ...
    float cShiftr = (ofNormalize(mlm, minMidLIn, maxMidLIn)*2)/3;
    cShiftr += ofNormalize(bm,minBassIn,maxBassIn)/3;
    float cShiftb = (ofNormalize(tm,minTopIn,maxTopIn)/2)+(ofNormalize(mlm,minMidIn,maxMidIn)/2);
...
float ofApp::calcSizeResponse(){
    ...
    float cleanMid = ofMap(mid,minMidIn,maxMidIn,minBassOut,maxBassOut,true);
    float bassMid = (ofNormalize(midL,minBassOut,maxBassOut))*cleanMid;//(ofMap(mid,minMidIn,maxMidIn,minBassOut,maxBassOut*2,true));//-midL;
    float topMid = (1-(ofNormalize(top,minTopOut,maxTopOut)))*cleanMid;
...
void ofApp::update(){
    ...
    for(int rep = 1; rep <= reps; rep++){
        ...
        if(rep<= repsFloor){
            ...
            nX += (twitchX*mappedMotion*fmodf(ofSignedNoise(tT),tempo/2));
            topColour.a = 100+rep*(155/repsCeil);
        }else if(rep > repsCeil){
            //movement based on tempo but inverted from other side of centre
            nX -= (twitchX*mappedMotion*fmodf(ofSignedNoise(tT),tempo/2));
            topColour.a = 255-((rep%repsCeil)*(155/(repsCeil)));
        }else if(rep == repsCeil){
            //no sideways movement for the centre critter
            nX -= ((1.5*twitchX)*mappedMotion*fmodf(ofSignedNoise(tT),tempo/2));
            topColour.a = 255;
        }
        //make the group head down with bass hit and up with top snaps
        //great, works pretty nicely first time
        yTendency = yTendency*peakDropRate;
        //so ease will be stickier if bigger
        float bassDrag =(ease*yTendency);
        bassDrag += (1-ease)*(ofMap(ofNormalize(bm,minBassIn,maxBassIn),0.575,1,0,1,true));
        float topLift = ((1-ease)*yTendency)+(ease*ofMap(ofNormalize(tm,minTopIn,maxTopIn),0.4,0.9,0,1));
        float bDrag = (bassDrag*(ofNormalize(bm,minBassIn,maxBassIn)))-topLift;
        yTendency = max(yTendency,bDrag);

        float halfHt =ofGetHeight()/2;
        nY = halfHt-(halfHt/3)+(((halfHt/4)*mappedMotion)*fmodf(ofSignedNoise(tT),tempo))+(((halfHt)*mappedMotionAbs)*yTendency);
        ofTranslate(nX,nY);
        ofRotateDeg(360/rep);
        ofSetColor(topColour);
        ofFill();
        ...
        for(int i = 0; i < clouds; i++){
            tx[i] += ((reactionLevel)*dt)*(mid+ofNormalize(top,minTopOut,maxTopOut)+ofNormalize(midL,minBassOut,maxBassOut));
            ty[i] += ((reactionLevel)*dt)*(mid+ofNormalize(top,minTopOut,maxTopOut)+ofNormalize(midL,minBassOut,maxBassOut));
...
void ofApp::keyPressed(int key){
    ...
    if(key == ' '){
        if(player.isPlaying() && tempoSampling){
            clearTempo();
        }
